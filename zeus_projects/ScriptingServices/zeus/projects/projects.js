...
var projectsDao = require('zeus/projects/dao/projectsDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
    ...
    if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
        if (id) {
            var entity = projectsDao.get(id);
            if (entity !== null) {
                sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(entity, null, 2));
            } else {
                sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'project_id\'=' + id);
            }
        } else if (count !== null) {
            var projectsCount = projectsDao.count();
            sendResponse(httpResponse, httpResponse.OK, 'text/plain', projectsCount);
        } else if (metadata !== null) {
            var projectsMetadata = projectsDao.metadata();
            sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(projectsMetadata, null, 2));
        } else {
            var projects = projectsDao.list(limit, offset, sort, desc);
            sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(projects, null, 2));
        }
    }
}
...
function handlePostRequest(httpRequest, httpResponse) {
    ...
    var id = projectsDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
    var entity = getRequestBody(httpRequest);
    var id = getIdParameter(httpRequest, xss);
    id = id !== null ? id : entity.project_id;
    if (id !== null) {
        if (projectsDao.get(id) !== null) {
        ...
        sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'project_id\' parameter is missing!');
    }
...
function getIdParameter(httpRequest, xss) {
    ...
    id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('project_id'));
    return id;
}
